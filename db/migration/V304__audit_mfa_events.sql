...
--
-- PREREQUISITE CHECK: This migration references AUDIT_ERROR_LOG for error logging.
-- Ensure audit_error_log.sql is loaded before this migration runs.
-- Fallback: If AUDIT_ERROR_LOG does not exist, errors are logged via DBMS_OUTPUT (visible in Flyway logs).
...
-- Partitioning Strategy: Range partition by created_at with monthly auto-partitioning.
-- Boundary (2025-11-01) is the assumed deployment date. The INTERVAL clause automatically creates
-- monthly partitions for any data with created_at >= 2025-11-01. The initial p_start partition
-- captures any pre-2025-11-01 data (typically empty on new deployments). This ensures that future
-- maintainers understand the partitioning rationale and can adjust the boundary if the deployment date changes.
PARTITION BY RANGE (created_at)
INTERVAL (NUMTOYMINTERVAL(1, 'MONTH'))
(
    PARTITION p_start VALUES LESS THAN (TO_DATE('2025-11-01', 'YYYY-MM-DD'))
);
...
-- HELPER PROCEDURE: Safely log errors with fallback to DBMS_OUTPUT if AUDIT_ERROR_LOG is unavailable
CREATE OR REPLACE PROCEDURE PRC_LOG_MFA_ERROR (
    p_error_code     VARCHAR2,
    p_error_message  VARCHAR2,
    p_context        VARCHAR2,
    p_procedure_name VARCHAR2,
    p_error_level    VARCHAR2 DEFAULT 'ERROR'
) AS
    l_table_exists NUMBER;
BEGIN
    -- Check if AUDIT_ERROR_LOG table exists in current schema
    SELECT COUNT(*)
    INTO l_table_exists
    FROM user_tables
    WHERE table_name = 'AUDIT_ERROR_LOG';
    
    IF l_table_exists > 0 THEN
        -- AUDIT_ERROR_LOG exists: write to it
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- If INSERT fails (permission, constraint, etc.), fall back to output logging
            DBMS_OUTPUT.PUT_LINE('[MFA_ERROR_LOG_FALLBACK] ' || p_error_code || ': ' || p_error_message || 
                                 ' | Context: ' || p_context);
        END;
    ELSE
        -- AUDIT_ERROR_LOG does not exist: log to DBMS_OUTPUT (visible in Flyway/migration logs)
        DBMS_OUTPUT.PUT_LINE('[MFA_ERROR_LOG_FALLBACK] ' || p_error_code || ': ' || p_error_message || 
                             ' | Context: ' || p_context);
    END IF;
EXCEPTION WHEN OTHERS THEN
    -- Catch-all: ensure logging failures never crash the procedure
    DBMS_OUTPUT.PUT_LINE('[MFA_ERROR_LOG_EXCEPTION] ' || SQLERRM);
END PRC_LOG_MFA_ERROR;
/
